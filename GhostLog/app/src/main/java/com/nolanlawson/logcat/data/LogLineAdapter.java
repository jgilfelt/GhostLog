...
//	}
    ...
    private View createViewFromResource(int position, View view, ViewGroup parent,
		...
		try {
			...
			logLine = LogLine.newLogLine("", true); //PreferenceHelper.getExpandedByDefaultPreference(context));
		}
		...
		int foregroundColor = Color.BLUE; //PreferenceHelper.getColorScheme(context).getForegroundColor(context);
		...
		float textSize = 8.0f; //PreferenceHelper.getTextSizePreference(context);
		...
		boolean extraInfoIsVisible = logLine.isExpanded() 
				&& true //PreferenceHelper.getShowTimestampAndPidPreference(context)
...
//		view.setBackgroundColor(selectedBackground);
	...
	private CharSequence ellipsizeFromCache(CharSequence str, int width, TextPaint paint) {
		...
		Integer maxLength = null; // PreferenceHelper.ellipsisLengthsCache.get(width);
